...
- Extend the `p8tool` entry point beyond the implemented text-cart `stats`, `listlua`, `listrawlua`, `listtokens`, `writep8`, `luamin`, and `luafmt` slices; check PNG listing/writing, output naming, prompting, presentation and argument edge cases against Python.

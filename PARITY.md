...
- `writep8`, `luamin`, and `luafmt` now match Python's output path notices and exact written text-cart bytes on both a simple and a complex vendored cart, including `luafmt --overwrite`, custom indent width, and `luamin --keep-all-names`. `luafmt` selects Python's AST formatter rather than the token formatter. Remaining writer filesystem/error edges and `luamin --keep-names-from-file` need comparison; direct Python PNG command checks remain unavailable because `pypng` is absent. The vendored Python writer does not prompt before overwrite.

...
- Remaining CLI argument and fatal-error edges, including command-specific diagnostics and build filesystem cases. Multi-file headings, ordered output/error continuation, and unsupported-extension statuses are compared with Python for working text-cart commands; mixed text/PNG paths have ordering checks. Text-cart `writep8`, `luamin`, and `luafmt` notices and written bytes match Python on simple and complex carts. Build minification honors both advertised name-preservation options for text and PNG outputs; the vendored Python build path ignores those flags.

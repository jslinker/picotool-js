...
- Remaining build filesystem cases. Multi-file headings, ordered output/error continuation, unsupported-extension statuses, parser failure statuses, malformed-cart continuation, option terminators/value forms, and help exit semantics are compared with Python; help prose is generated natively in pure JavaScript rather than copied byte-for-byte from Python argparse. Writer success bytes and the input-load versus output-write failure boundary match Python. Build minification honors both advertised name-preservation options for text and PNG outputs; the vendored Python build path ignores those flags.

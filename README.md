...
- Remaining class-level AST metadata parity. Public AST fields, token values, spans, token-group layouts, regenerated token streams, walker traversal traces, walker-driven token edits, and replacement of names, binary operands, table fields, and `if` blocks are compared recursively with Python on all accepted vendored parser-test inputs, including fragment/residual cases, and text carts. The implementation remains pure JavaScript; optional named-constructor declaration signatures are provided for consumers.

...
- Remaining CLI argument and fatal-error edges, including command-specific diagnostics and build name-preservation options. Multi-file headings, ordered output/error continuation, and unsupported-extension statuses are compared with Python for working text-cart commands; mixed text/PNG paths have ordering checks. Text-cart `writep8`, `luamin`, and `luafmt` notices and written bytes match Python on simple and complex carts, including overwrite, custom indent width, and `luamin --keep-names-from-file`.

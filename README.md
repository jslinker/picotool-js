...
- Remaining `BaseASTWalker` child-replacement and custom-handler mutation edges. Public AST fields, token values, spans, token-group layouts, regenerated token streams, walker traversal traces, and walker-driven token edits are compared recursively with Python on all accepted vendored parser-test inputs, including fragment/residual cases, and text carts. The implementation remains pure JavaScript; optional named-constructor declaration signatures are provided for consumers.
